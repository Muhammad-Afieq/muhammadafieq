# WELCOME TO MY ePORTFOLIO

# PROFILE  

![WhatsApp Image 2026-01-19 at 08 51 00](https://github.com/user-attachments/assets/a568fa90-704d-45a7-b571-4b5e2c44fa5d)
 
   NAME: MUHAMMAD AFIEQ BIN HASBULLAH                                                                                                          
   SECTION:05                                                                                                                                        
   COURSE CODE:SECP1513                                                                                          
   COURSE NAME:TECHNOLOGY AND INFORMATION SYSTEM                                                   
   LECTURER:ASSOCIATE PROF DR.AZURAH BINTI A SAMAH                                                  

 # SUMMARY

   STUDENT AT UNIVERSITI TEKNOLOGI MALAYSIA | BACHELOR OF COMPUTER SCIENCE(NETWORK AND SECURITY)
   
# CORE
 # EDUCATION
   Bachelor of Computer Science in Network and Security with Honours Universiti Teknologi Malaysia | 2025-2029                                                                            
   Engineering and Technology at Kolej Mara Kuala Nerang  | 2024-2025                                                        
         CGPA : 3.96                                                                                                
         MUET : 4.00                                                                                  
   Science Elective at SMK Datuk Haji Ahmad Badawi | 2019-2024                                          
         SPM : 6A 3B                                                               
   Sekolah Kebangsaan Bertam Perdana | 2014-2018                                                                                    
         UPSR : 6B 2C 

 # POSITION
   Bachelor of Computer Science in Network and Security with Honours Universiti Teknologi Malaysia | 2025-2029                                                                
       Committee members for Kolej Tun Razak as multimedia                                                                            
   Foundation of Engineering and Technology at Kolej Mara Kuala Nerang | 2024-2025                                           
       Active member in Chess club                                                                                            
   Pre-Science: Science Elective at SMK Datuk Haji Ahmad Badawi | 2019-2024                                                
       Deputy President of the School Scout                                                                                      
  # Skill                                                                                                               
   Coding: C++                                                                                                              
   Web development: HTML                                                                                                

# AWARD
   Gold Medal at Jamboree Sambutan 100 Tahun Pengakap Kedah 2022                                                                 

# PROJECT
   SECP1513 TECHNOLOGY AND INFORMATION SYSTEM | 202526/1                                                           
      Design Thinking Project | AutiCont                                                                 
   SECR1013 DIGITAL LOGIC | 202526/1                                                                                   
      Final Project | 3 BIT UP AND DOWN LIFT SYSTEM T FLIP FLOP                                                                                                                        

# ASSIGNMENT
   Industry Talk at UTM Digital Open Day | 29 october 2025:                                                                                                                                                                                                                                                                                    
On that day, I was given the opportunity to attend an industrial talk by Mr. Helmee Bin Yaakob on system development. From the talk, I learned how UTM Digital operates efficiently to support students and how its systems function. He also shared details about project failures, which helped prepare me for challenges I may face in university and in future workplaces.

We also had the opportunity to interview one of the employees, Muhammad Farid Bin Mohd Ariffin, who works as a system developer for research and development information systems. Through this interview, many of our questions and curiosities were answered clearly, which further enhanced my understanding of real-world system development.

As an improvement, longer and more interactive question-and-answer sessions would allow students to gain deeper insight into system development practices and industry challenges. Overall, this experience gave me a more realistic view of professional system development, increased my motivation to learn, and highlighted the importance of communication, teamwork, and continuous learning for a future career in technology.
<img width="1280" height="720" alt="image" src="https://github.com/user-attachments/assets/3843c882-de6a-48ba-ac33-7ed0297c3f3a" />
<img width="960" height="1280" alt="image" src="https://github.com/user-attachments/assets/daf6368d-9c61-403c-b113-b98ee0a073bb" />

                                                                                                                                                                              
PC Assemble (Lab) | 12 November 2025:                                                                                                                            
PC assembly was carried out in groups of five. Before starting the assembly process, we were given instructions and required to disassemble the PC first. From this activity, I gained a clear understanding of where each hardware component is placed and its specific function in a personal computer (PC). I realized that every hardware component plays an important role in ensuring the PC works efficiently and functions properly as a complete system.

As a suggestion for improvement, explaining common assembly mistakes and basic troubleshooting techniques would make the activity more realistic and helpful, while also enhancing students’ knowledge of PC hardware. Other than that, this activity also improved my teamwork skills, as assembling a PC requires cooperation, communication, and patience among group members. Overall, this experience made learning more engaging and helped me appreciate the importance of hardware knowledge in computing.
<img width="1280" height="960" alt="image" src="https://github.com/user-attachments/assets/c13e18fe-18ed-4304-9257-234c25ee9374" />
<img width="810" height="1080" alt="image" src="https://github.com/user-attachments/assets/0feb0c13-3c4a-4dfd-8b91-7bd7e83b485f" />                                                                                  

Industry Talk for PPG Industies | 20 November 2025:                                                                                                 

Attending the industrial talk by PPG gave me valuable insight into how a global company operates using data analytics, cloud systems, and SAP to manage worldwide operations. I gained a better understanding of how different departments, such as the Global Infrastructure COE and Data Analytics COE, work together to support decision-making and ensure operational efficiency. The talk also introduced me to real industry tools and practices, including cloud platforms, databases, SQL, DevOps, and ERP systems like SAP S/4HANA, which helped me see how the theoretical knowledge from class is applied in a real-world setting. Learning about data-driven decision making and system integration gave me a clearer picture of how technology supports large-scale business operations.

One area for improvement could be including more interactive sessions, such as longer Q&A segments or short discussions with the speakers, allowing students to ask detailed questions and understand specific roles and technologies better. Adding real case studies or live demonstrations of systems like SAP or data analytics dashboards could also make the session more engaging and easier to grasp, especially for students new to industry-level systems.

Overall, the talk increased my interest in how technology is applied in large organizations and gave me a better sense of potential career paths in areas such as data analytics, system development, and IT infrastructure. It also highlighted the importance of continuously upgrading skills in areas like data analysis, cloud computing, and system integration. Connecting academic learning with real-world applications was meaningful, motivating me to better prepare for internships and future professional opportunities.
<img width="837" height="1183" alt="image" src="https://github.com/user-attachments/assets/68ae1d0f-8f5e-4fb1-98f4-cdbab3b47f7e" />

                                                                                                                               
Industry Talk for Serunai Commerce Sdn Bhd Industry | 18 December 2025:                                                                                                                                
The industrial talk from Ts.Haji Abdul Halim Bin Abdul Muttalib gave me a clear view of how project management and system development work in real industry settings. I realized that a successful project isn’t just about technical skills—it also depends on good planning, clear communication, and teamwork. Learning about methodologies like Waterfall and Agile made me see that the right approach depends on the project’s needs, not personal preference. It also helped me understand the SDLC better and how each phase plays a role in reducing errors and ensuring systems run smoothly.

One way the talk could be improved is by including more real-life case studies or showing actual projects from the industry. Adding interactive elements like Q&A sessions, group discussions, or mini activities could make it more engaging and help students connect theory to practice. Hearing about the challenges professionals face and how they solve them would also add a lot of value.

Overall, the talk motivated me to focus not just on technical skills but also on soft skills like communication, teamwork, and time management. It reminded me that AI tools are best used to support learning, not replace understanding the basics. This experience has encouraged me to be proactive in preparing for industry expectations and to keep improving both my technical abilities and personal development.

                                                                                                                                          
Industry Visit at Biocon | 23 December 2025:                                                                                                                                                              
I had the chance to visit Biocon, where students were split into three groups, and I joined the DP (Drug Product) Operation department led by Mr. Jaya. The visit gave me a clear look at how insulin is produced and packaged in an industrial setting.

Mr. Jaya explained the four main steps of insulin production. First, formulation, where insulin is prepared with precise composition and quality control. Next is filling, where containers are accurately filled under strict sterile conditions. This is followed by visual inspection, ensuring every product meets safety and quality standards. Finally, packaging seals, labels, and prepares the insulin for distribution. Seeing how these steps work together helped me understand the careful planning and precision required in pharmaceutical manufacturing.

Photography was limited for safety and privacy reasons, but Mr. Jaya made the session engaging by encouraging questions and sharing his industry experience. He also gave useful advice on developing skills like discipline, attention to detail, teamwork, and continuous learning.

One improvement could be providing more visual materials, like diagrams or videos, to supplement the explanations when photography isn’t allowed. This would make it easier for students to understand complex processes.

Overall, the visit was a valuable learning experience. It helped me connect theory to real-world pharmaceutical operations and inspired me to strengthen both my technical knowledge and soft skills to prepare for future careers in technology and healthcare.
<img width="1000" height="750" alt="image" src="https://github.com/user-attachments/assets/3d61bce1-5701-4f23-8e14-864768f644e6" />
<img width="1000" height="750" alt="image" src="https://github.com/user-attachments/assets/601eb71b-f9d8-47e5-918f-b21d1db00939" />

                                                                                                                                           
Design Thinking (Low Fidelity Prototype) | Semester 1:                                                                                                                                
The AutiCont project does a great job applying design thinking to help autistic children and their guardians, but there’s room for improvement. One idea is to make the app more interactive by adding customizable features, like adjustable alert thresholds or personalized calming suggestions based on each child’s behavior. This could help the app better meet individual needs and reduce false alerts.

Future versions could also benefit from working closely with therapists or psychologists to improve stress detection and intervention recommendations. Longer testing with real users would help uncover limitations and make the system more reliable. From my own learning experience, the project highlighted the importance of not relying too much on AI-generated ideas and focusing more on research and critical thinking to create original solutions.

Overall, this project reinforced that technology should prioritize empathy, responsibility, and real-world impact. Combining technical skills with human-centered design can lead to solutions that genuinely improve lives. This experience has encouraged me to be more reflective, creative, and thoughtful when designing systems in the future.

  
   


   
  
 
